...
const CATEGORIES = ["film", "tvshow", "trailers", "clips"];
...
async function makeTestClip(job) {
	...
	const draw = FONT
		? ["-vf", `drawtext=fontfile='${FONT.replace(":", "\\:")}':text='DRIVN TEST REEL':fontsize=48:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2:box=1:boxcolor=black@0.5:boxborderw=12`]
		: [];
	jline(job, FONT ? `drawtext via ${FONT}` : "no system font found — plain testsrc2 (still valid)");
	let enc = await ff(["-f", "lavfi", "-i", "testsrc2=size=640x360:rate=24",
		"-f", "lavfi", "-i", "sine=frequency=440", "-t", "12", ...draw, ...VIDEO_ARGS, ogv], job, 12);
	if (enc.code !== 0 && draw.length) { // drawtext hiccup → fall back to plain pattern
		jline(job, "drawtext failed — retrying plain testsrc2");
		enc = await ff(["-f", "lavfi", "-i", "testsrc2=size=640x360:rate=24",
	...
	const row = makeRow({ id, category: cat, title: "DRIVN Test Reel", runtime_seconds: +duration.toFixed(2),
...
const HELP = {
	...
	endpoints: [
		...
		"GET    /api/scan                      -> unconverted sources in the 4 category folders + mp3s in music/",
