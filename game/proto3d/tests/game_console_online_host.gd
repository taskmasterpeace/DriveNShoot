## Real ENet HOST proof for the Game Deck. The client drives DIAL TANKS seat 1;
## this authority applies that input, publishes a snapshot and one result, then
## requires the client to acknowledge converged state and one ledger write.
extends Node

const PORT := 24779
const SESSION := "console-wire-proof"
const GAME_ID := "dial_tanks"
const SEED := 73421

var net: ProtoNet = null
var deck: Node = null
var remote_peer := 0
var saw_remote_input := false
var sent_snapshot := false
var client_ack := false
var remote_players: Dictionary = {}


func notify(_text: String) -> void:
	pass


func _ready() -> void:
	net = ProtoNet.create(self)
	add_child(net)
	net.peer_joined.connect(_on_peer_joined)
	if not net.host(PORT):
		print("CONSOLE HOST: BIND FAILED")
		get_tree().quit(1)
		return
	print("CONSOLE HOST: LISTENING")
	get_tree().create_timer(15.0).timeout.connect(func() -> void:
		print("CONSOLE HOST: WATCHDOG FAIL")
		get_tree().quit(1))


func _on_peer_joined(peer_id: int) -> void:
	remote_peer = peer_id
	var members: Array = [1, remote_peer]
	if not net.arcade.begin_session(SESSION, GAME_ID, 1, members):
		print("CONSOLE HOST: SESSION FAIL")
		get_tree().quit(1)
		return
	deck = ProtoGameDeck.create(self)
	add_child(deck)
	deck.set_process(false)
	deck.attach_net(net.arcade)
	net.arcade.input_received.connect(_on_remote_input)
	net.arcade.event_received.connect(_on_remote_event)
	var context := {"source": "session", "online": true, "session_id": SESSION,
		"local_peer_id": 1}
	var seats: Array = [
		{"seat": 0, "peer_id": 1, "device": -1, "profile_id": "host"},
		{"seat": 1, "peer_id": remote_peer, "device": -1, "profile_id": "remote"},
	]
	if not deck.launch(GAME_ID, context) or not deck.start(SEED, seats):
		print("CONSOLE HOST: DECK START FAIL")
		get_tree().quit(1)
		return
	print("CONSOLE HOST: MATCH READY peer=%d" % remote_peer)


func _on_remote_input(peer_id: int, _input_tick: int, state: Dictionary) -> void:
	if peer_id != remote_peer or saw_remote_input:
		return
	var move: Vector2 = state.get("move", Vector2.ZERO)
	saw_remote_input = move.y < -0.9 and int(state.get("seat", -1)) == 1
	if not saw_remote_input:
		return
	# Three authority ticks trigger the deck's stock snapshot cadence.
	deck.process_tick()
	deck.process_tick()
	deck.process_tick()
	var tanks: Array = deck.cartridge.get("tanks")
	var remote_tank: Dictionary = tanks[1]
	sent_snapshot = int(deck.cartridge.get("tick")) == 3
	print("CONSOLE HOST: REMOTE INPUT APPLIED pos=%s" % str(remote_tank.get("pos")))
	call_deferred("_publish_result")


func _publish_result() -> void:
	if not bool(deck.cartridge.debug_force_finish()):
		print("CONSOLE HOST: RESULT FINISH FAIL")
		get_tree().quit(1)
		return
	var result: Dictionary = deck.cartridge.get("last_result")
	# Deliberately transmit the same result again. The receiver must reject it.
	get_tree().create_timer(0.15).timeout.connect(func() -> void:
		net.arcade.send_result(result))
	print("CONSOLE HOST: RESULT PUBLISHED")


func _on_remote_event(peer_id: int, event: Dictionary) -> void:
	if peer_id != remote_peer or String(event.get("type", "")) != "client_audit":
		return
	client_ack = bool(event.get("snapshot_ok", false)) \
		and int(event.get("ledger_count", 0)) == 1 \
		and int(event.get("result_signals", 0)) == 1
	var host_ledger_count := (deck.ledger.recent_results as Array).size()
	var ok := saw_remote_input and sent_snapshot and client_ack and host_ledger_count == 1
	print("CONSOLE HOST: CLIENT ACK snapshot=%s ledger=%d signals=%d" % [
		str(event.get("snapshot_ok", false)), int(event.get("ledger_count", 0)),
		int(event.get("result_signals", 0))])
	print("CONSOLE HOST: %s" % ("ALL CHECKS PASSED" if ok else "FAILURES PRESENT"))
	get_tree().create_timer(0.2).timeout.connect(func() -> void:
		get_tree().quit(0 if ok else 1))
