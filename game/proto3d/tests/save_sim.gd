...
func _ready() -> void:
	...
	main.homebase.interact(main) # WALLS I (6 scrap + 10 jack)
	...
	main.backpack.remove("jack", jack0)
	main.daynight.day = 1
	...
	_check("(the wreck is real: dog gone, pockets empty)", main.backpack.count("jack") == 0 and not is_instance_valid(dog))
	...
	_check("the pockets came back (%d jack)" % main.backpack.count("jack"), main.backpack.count("jack") == jack0)
