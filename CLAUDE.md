...
| Paperdoll gear | `gear.gd` + `character.gd` + `data/equipment.json` | THE 19-SLOT PAPERDOLL: ProtoGear catalog (6 armor/7 clothing/6 accessory), every slot bare by default, folds JSON additively (a new gear = a ROW); registers each piece as a usable+priced ITEM + seeds loot (found & bought). EFFECT-WIRED via clean single-consumers: armor SOAK at the `take_wound` choke (clamped 0.75) · carry (back/belt→`carry_cap`) · stealth (coat→`stealth_detect_mult`) · unarmed (ring→`unarmed_dmg_mult`) · reload (sash→`reload_mult`) · repair (bracelet→`repair_mult`). USE wears one-per-slot, rides the save. Left for later (need new systems, not a wire): shirt/footwear/face/ear/talisman effects + faction-ID (respect-ledger, a design call). Sheet UI = owner's lane. `equip_sim` (26 checks) |
