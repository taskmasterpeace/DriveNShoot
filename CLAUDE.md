...
- **project.godot COMMENT LAW (2026-07-16):** never put multi-line or double-quoted comments in `project.godot`. An addon's config re-serializer strips whitespace + newlines and swallows the NEXT line into the comment — it silently commented OUT the `DialogueManager` autoload and dropped plugins from `enabled`. Looks exactly like "an addon corrupted my project"; it's latent and any plugin install triggers it. Keep `[autoload]` comment-free (context lives in git/docs instead).
- **THE EDITOR BRIDGE (2026-07-16):** Godot AI 2.9.0 (`game/addons/godot_ai/`, server on 127.0.0.1:8000) exposes the LIVE editor to Claude Code over MCP — `editor_screenshot`, `project_run`, `logs_read`, scene/node/script ops, and `game_manage` input driving. The old `godot_mcp` (9080) is disabled — don't run both. AssetLib has 3+ addons named "Godot AI": the right one is **dlight / version 2.9.0** ("connects MCP-compatible AI assistants to a live Godot editor"), NOT the v0.1.0 chat dock.
