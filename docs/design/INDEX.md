...
|---|---|---|---|---|
| **THE_COUNTRY_PLAN** | the 3-arc country ladder: 1A painted relief bands + climbing roads (grade-capped road elev bake) · 1B rivers/bridges/60 overpasses + water_depth_at is BORN · 2 town landmarks/farm belts/exit-service billboards/ecotones · 3 districts→engine + ghost sites; §0 = the research corrections (relief v1 + M4b furniture ALREADY shipped — don't rebuild) | **BANKED (2026-07-14, research-verified)** | Arc 1A: `relief_paint_sim` + driven-climb + Florida/Meridian byte-flat | TERRAIN_RELIEF (design), GROUND_INTEGRITY (shipped law), RDS elev channel (shipped) |
...
| **THE_AMERICAN_ROAD** | junction/exit/address laws, road hierarchy+dirt, corridor look, milestones M0–M9 | **M0–M4a SHIPPED** (92fbd87 wire · M1 junctions/gaps/yaw-bug/exit-peel · 5be6ad4 M2 integrity · 978c8c8 Meridian=EXIT 9 · 3291bb3 two-tier towns · 1e9a194 M3b network fill+dirt layer · 4730500 M4a corridor kit) + **THE CITY & ROAD OVERHAUL (2026-07-18, owner /goal "improve every city")**: DIAMOND INTERCHANGES (mirror-ramp-crosses-highway bug DEAD, 85→0; every exit has a return ramp, one-way 66→0; far-side served by an at-grade cross-street into town) · HEAL DEAD ENDS (82 healed, wild dead-ends 13→0 — the 8 mid-map interstates reach their cities) · road_graph co-located-node fix (interchanges no longer split a highway) + **THE ROAD SYSTEM PASS (2026-07-19)**: **M2 THE OVERPASS BUILT** (the 152 `separated_pending` crossings rendered NOTHING — the minor road now bridges the major on a real deck with drivable approach ramps, piers and rails; median stays intact) · bends render whole (`road_segments_near` — 459 segments were never drawn) · intersection slabs ALIGNED to their road (were axis-aligned squares at rot 0) · ramp_rejoin + end_cap dressed (618 nodes rendered nothing) · MapForge stopped destroying road fields on edit → **M4b/M5 next** | M4b mile markers/shields; M5 furnished interiors (ProtoInteriorSkin — a light furnisher shipped, see BUILDING_BOOK) | conformance harness (MAPMAKER GAP 1) |
| **THE_ROAD_KIT_AND_LANE_GRAPH** | the modular road kit (profile-as-socket, junctions discrete, runs procedural) + THE LANE GRAPH: lane rows, the oncoming rule, junction connectors with foes/response conflict masks | **FOUNDATION SHIPPED (2026-07-19)** — 2,034 lanes · 19,972 connectors · 26,446 conflict pairs, baked lazily ~1.1 s; lane_graph_sim 18/0 | agents ADOPT it (MT): traffic `_maybe_transfer` at nodes, IDM+CAH follower, pure-pursuit on lane centrelines, gap acceptance | AMERICAN_ROAD; supersedes nothing — it is MT's substrate |
| **THE_BUILDING_BOOK** | canonical catalog, interiors law, mission schema, MULTI-USE law, strategies | BANKED + **FURNISHED SHELLS SHIPPED (2026-07-18)**: ProtoStructureBuilder dresses every enterable shell with category-matched interactable furniture (residential→fridge/closet, civic_law→locker/safe, medical→cabinet…) — the 682 enterable placements across all 59 towns are real walk-ins, no longer empty boxes (structure_data_sim 37/0) | M0 materialize wire ✓; M3c mission fields; M5 multi-floor/roof-hide interiors | AMERICAN_ROAD milestones |
