...
| **THE_INFECTED** | the failed trials: taxonomy rows, herds-as-cells, the Choir zone law + `infection_pressure` writer, BITE FEVER, quarantine, the PIT/PREY laws, the Trials films | **I1 part 1 SHIPPED** (117cc1c: ProtoInfected walks/pools, THE BODY LAW live w/ infection=1.0, BITE FEVER, the every-shooter gunfire emitter, sprint footfalls; infected_sim 10/10 + 40-body perf probe) → I1 part 2 = the Choir (benning flip + registry + dial bleed + balk + F-IP + choir_zone_sim + carousel2 retarget) | I2 taxonomy/law after | none for I1; P0 ✓ |
