...
| W2 | OCEAN LOOK: per-chunk water sheets + the researched shader (two-tone flat + depth foam + bob; reads top-down); Miami waterfront visible from the depot | W | **STAGED** — `water.gdshader` written (CC0+MIT cited in header); sheet placement + painted edge-foam strips land after the suite stamp (no world_stream edits mid-suite) | acceptance shots (both termini) graded by fresh-context judge |
...
| L1 | THE LOOK walkthrough as a sim: depot → board (real E) → T-skip the line → Miami platform → DRIVE into the Atlantic under real throttle → engine drowns, player swims out | R+W | **STAGED** — `seaboard_walkthrough_sim` written; first run after the suite stamp | `seaboard_walkthrough_sim` green |
